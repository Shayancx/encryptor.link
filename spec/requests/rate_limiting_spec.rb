...
RSpec.describe "Rate Limiting", type: :request do
  ...
  describe 'decryption data endpoint' do
    let(:payload) { create(:encrypted_payload, :many_views) }

    it 'allows 30 requests per minute' do
      30.times do |i|
        get "/#{payload.id}/data", headers: { 'REMOTE_ADDR' => '1.2.3.5' }
        expect(response.status).not_to eq(429), "Failed on request #{i+1}"
      end
  ...
  describe 'response headers' do
    it 'includes Retry-After header when rate limited' do
      ...
      11.times do
        post '/encrypt', params: valid_params, headers: { 'REMOTE_ADDR' => '1.2.3.7' }
      end
