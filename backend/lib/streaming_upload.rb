...
module StreamingUpload
  ...
  LOG_PATH = File.expand_path('../logs', __dir__)
  FileUtils.mkdir_p(LOG_PATH)
  LOGGER = Logger.new(File.join(LOG_PATH, 'streaming.log'), 'daily')
  ...
  class << self
    ...
    def store_chunk(session_id, chunk_index, chunk_data, iv)
      ...
      raise "Invalid session: #{session_id}" unless File.exist?(session_path)
      ...
      File.open(metadata_file, 'r+') do |file|
        ...
        if chunk_index.negative? || chunk_index >= metadata['total_chunks']
    ...
    def finalize_session(session_id, salt)
      ...
      raise "Session not found: #{session_id}" unless File.exist?(metadata_file)
      ...
      metadata['total_chunks'].times do |i|
        ...
        raise "Chunk file missing or empty: #{i}" unless File.exist?(chunk_file) && File.size(chunk_file).positive?
      end
    ...
    def get_file_info(file_id)
      ...
      raise "File not found: #{file_id}" unless file_record

      raise 'File is not chunked' unless file_record[:is_chunked]

      File.open(file_record[:file_path], 'rb') do |f|
        header_size = f.read(4).unpack1('N')
    ...
    def read_chunk(file_id, chunk_index, password)
      ...
      raise 'Invalid password' unless Crypto.verify_password(password, file_record[:salt], file_record[:password_hash])
      ...
      File.open(file_record[:file_path], 'rb') do |f|
        ...
        raise 'Chunk index out of range' if chunk_index >= header['total_chunks']

        # Skip to requested chunk
        chunk_index.times do
          iv_size = f.read(4).unpack1('N')
          f.seek(iv_size, IO::SEEK_CUR)
          chunk_size = f.read(4).unpack1('N')
...
Thread.new do
  loop do
    begin
      ...
      StreamingUpload::LOGGER.error "Cleanup error: #{e.message}" if defined?(StreamingUpload::LOGGER)
    end
    sleep 300
  end
end
